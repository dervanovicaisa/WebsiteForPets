...
session_start();
$idMacke=$id=$rasa=$opis=$url="";
if(isset($_GET['id'])) {
    $idMacke = $_GET['id'];
    $res = mysqli_query($con, "select * from macke where id=$idMacke");
...
if (isset($_POST['submit'])) {
    $opis = $_POST['opis'];
    $sql = "update macke set opis ='" .$opis ."' where id='" .$idMacke."'";
...
<div class="containerProfil">
    <div class="desnaStrana">
        <form action="urediMacke.php?id=<?php echo $idMacke; ?>" method="post">
            <div class='card-group col-md-10'>
                <div class='card' name="id" id='<?php echo $idMacke; ?>'>
                    <img class='card-img-top' src="<?php echo $url; ?>" alt='Card image cap'>
                    <div class='card-body'>
                        <h3 class='card-title text-center'><?php echo $rasa; ?></h3>
                        <textarea class='card-text'  name="opis" cols='61' rows='15'><?php echo $opis; ?></textarea>
                        <p class='card-text'> <small class='text-muted'><?php echo $rasa; ?></small></p>
                    </div>
                </div>
            </div>
